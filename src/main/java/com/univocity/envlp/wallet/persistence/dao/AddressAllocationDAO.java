...
@Repository
public class AddressAllocationDAO extends BaseDAO {
	...
	public void createDefaultAllocations(WalletSnapshot wallet) {
		List<Long> missingAccounts = db().queryForList("SELECT account_idx FROM wallet_account WHERE wallet_id = ? AND account_idx NOT IN (SELECT account_idx FROM address_allocation WHERE wallet_id = ?)", Long.class, wallet.getId(), wallet.getId());
		if (!missingAccounts.isEmpty()) {
			List<Object[]> batch = new ArrayList<>();
			missingAccounts.forEach(accountIndex -> batch.add(new Object[]{wallet.getId(), accountIndex}));
			db().batchUpdate("INSERT INTO address_allocation (wallet_id, account_idx, derivation_idx) VALUES (?,?,0)", batch);
		}
	}

	public AddressAllocation allocateNextAccount(WalletSnapshot wallet) {
		return allocateNextAddress(wallet, -1L);
	}

	public void persistPaymentAddress(AddressAllocation allocation) {
		if (db().update("UPDATE address_allocation SET payment_address = ? WHERE wallet_id = ? AND account_idx = ? AND derivation_idx = ? AND payment_address IS NULL", allocation.getPaymentAddress(), allocation.getWalletId(), allocation.getAccountIndex(), allocation.getDerivationIndex()) == 0) {
	...
	public AddressAllocation allocateNextAddress(WalletSnapshot wallet, long accountIndex) {
		String query;
		...
		AddressAllocation allocation = db().queryForFirstObject(query, rowMapper, wallet.getId());
		if (allocation == null) {
			if (accountIndex < 0) {
		...
		if (allocation.isAvailable()) { //available address, claim it.
			db().update("UPDATE address_allocation SET available = 0, claimed_at = ? WHERE wallet_id = ? AND account_idx = ? AND derivation_idx = ?", now, wallet.getId(), allocation.getAccountIndex(), derivation);
			db().insert("INSERT INTO address_allocation (wallet_id, account_idx, derivation_idx) VALUES (?,?,?)", wallet.getId(), allocation.getAccountIndex(), derivation + 1);
		} else {
		...
		allocation = db().queryForObject("SELECT * FROM address_allocation WHERE wallet_id = ? AND account_idx = ? AND derivation_idx = ?", rowMapper, wallet.getId(), allocation.getAccountIndex(), derivation);
		return allocation;
	}

	public List<AddressAllocation> getAddresses(WalletSnapshot wallet, long accountIndex) {
		String query;
		...
		return db().query(query, rowMapper, wallet.getId());
	}
}
